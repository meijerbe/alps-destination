...
2. Open **SQL Editor → New query**, plak de inhoud van [`supabase/schema.sql`](supabase/schema.sql)
   ...
   Het script is veilig om zo vaak te draaien als je wilt, en controleert zichzelf: staat er
   iets niet goed, dan krijg je een foutmelding in plaats van stil half werk. Onderaan hoort
   een tabel met **twaalf rijen** te verschijnen, vier policies per tabel. Zie je er minder,
   of krijg je in de app *"de database laat dit nog niet toe"*, draai het dan opnieuw —
   dat is precies waar dit script voor is.
